...
import { fetchAllRequestByRescuerAPI, filterStatusRequestAPI, searchRequestByKeywordAPI } from "../../service/api.service";
...
const ManageFilterRequest = (props) => {
    ...
    const { user } = useAuth();
    const myRequestsCount = dataRequest.filter(item => item.rescuer?.email === user?.email).length;
